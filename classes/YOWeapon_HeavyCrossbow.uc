class YOWeapon_HeavyCrossbow extends AOCWeapon_HeavyCrossbow;
...
simulated function ChangeProjectileType(class<AOCProjectile> Proj)
{
	...
	// Handle extra ammmmmmoooooooooooo
	// Check tertiary weapon if we're a crossbow -- keeping it in AOCRangeWeapon just in case we expand to other weapons
	if (AOCOwner.PawnInfo.myTertiary == class'YOWeapon_ExtraAmmo')
	{
		AmmoCount += class'YOWeapon_ExtraAmmo'.default.ExtraAmmo;
	}
...
DefaultProperties
{
	...
	AttachmentClass=class'YOWeaponAttachment_HeavyCrossbow'
	...
	ConfigProjectileBaseDamage[3]=(Damage=120,InitialSpeed=7225.0,MaxSpeed=7650.0,AmmoCount=12,InitialGravityScale=0.25,Drag=0.000003,PitchCorrection=0)
	...
	WeaponFontSymbol="d"
	WeaponReach=100
	WeaponLargePortrait="SWF.weapon_select_crossbowheavy"
	WeaponSmallPortrait="SWF.weapon_select_crossbowheavy"
	...
	ReloadAnimations[0]=(AnimationName=3p_crossbow_reloadheavy,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.heavy_crossbow_Reload',bFullBody=true,bCombo=false,bLoop=false,bForce=false,fModifiedMovement=0.0,fAnimationLength=4.5,fBlendInTime=0.1,fBlendOutTime=0.20,bLastAnimation=true,bPlayOnWeapon=true,bAttachArrow=1)
