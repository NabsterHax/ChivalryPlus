class YOWeapon_Torch extends AOCWeapon_Torch; //UNUSED!
...
simulated state Active
{
	...
	simulated event BeginState(Name PreviousStateName)
	{
		...
		NotifyAmmoConsume();
		AOCInventoryManager(AOCOwner.InvManager).bCanSwitchWeapons = false;	
		if (AmmoCount <= 0)
		{
			AOCBaseHUD(AOCPlayerController(AOCOwner.Controller).myHUD).TurnOffCrosshair();
			AOCInventoryManager(AOCOwner.InvManager).bCanSwitchWeapons = true;	
			InvManager.RemoveFromInventory(AOCOwner.CarriedTorch);
			//AOCPlayerController(AOCOwner.Controller).ReceiveChatMessage("", "SWITCHING FROM TORCH, ACTIVE",EFAC_ALL,true);
			//AOCInventoryManager(InvManager).SwitchWeapon(AOCInventoryManager(InvManager).CurrentWeaponGroup);

			if (AOCOwner.bShieldEquippedBeforeTorch)
				AOCOwner.EquipShield(true);

			// Notify PC we've finished using torch
			if (Role == ROLE_Authority && AOCPlayerController(AOCOwner.Controller) != none)
				AOCPlayerController(AOCOwner.Controller).NotifyUseDropTorch();
		}
		else
		{
			// Notify PC we're using a torch
			if (Role == ROLE_Authority && AOCPlayerController(AOCOwner.Controller) != none)
				AOCPlayerController(AOCOwner.Controller).NotifyPickUpTorch();
		}
	}
}
...
simulated state Release
{
	simulated event EndState(Name NextStateName)
	{
		...
		if (AmmoCount <= 0)
		{
			...
			AOCInventoryManager(AOCOwner.InvManager).bCanSwitchWeapons = true;	
			InvManager.RemoveFromInventory(AOCOwner.CarriedTorch);
			//AOCPlayerController(AOCOwner.Controller).ReceiveChatMessage("", "SWITCHING FROM TORCH, RELEASE",EFAC_ALL,true);
			//AOCInventoryManager(InvManager).SwitchWeapon(AOCInventoryManager(InvManager).CurrentWeaponGroup);
...
DefaultProperties
{
	...
	AttachmentClass=class'YOWeaponAttachment_Torch'
	...
	ConfigProjectileBaseDamage[5]=(Damage=0,InitialSpeed=1275.0,MaxSpeed=1700.0,AmmoCount=1,InitialGravityScale=1.0,Drag=0.0000013,PitchCorrection=60.0)
	
	iFeintStaminaCost=0
	WeaponFontSymbol="L"
	WeaponReach=100
	WeaponLargePortrait="SWF.weapon_select_torch"
	WeaponSmallPortrait="SWF.weapon_select_torch"
	...
	WindupAnimations[0]=(AnimationName=3p_1hsharp_throwdowntoup,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.BroadDagger_windup',bFullBody=False,bCombo=False,bLoop=False,bForce=false,fModifiedMovement=0.7,fAnimationLength=0.0,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false)
	WindupAnimations[1]=(AnimationName=3p_1hsharp_slash02downtoup,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.BroadDagger_windup',bFullBody=False,bCombo=False,bLoop=False,bForce=false,fModifiedMovement=0.7,fAnimationLength=0.0,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false)
	WindupAnimations[2]=(AnimationName=3p_1hsharp_stabdowntoup,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.BroadDagger_windup',bFullBody=False,bCombo=False,bLoop=False,bForce=false,fModifiedMovement=0.7,fAnimationLength=0.0,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false)
	...
	ReleaseAnimations[5]=(AnimationName=3p_1hsharp_shoverelease_new,ComboAnimation=,AssociatedSoundCue=,bFullBody=True,bCombo=False,bLoop=False,bForce=false,UniqueShieldSound=none,fModifiedMovement=1.0,fAnimationLength=0.3,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false,bUseRMM=true)
	...
	ReloadAnimations[0]=(AnimationName=3p_1hsharp_throwreload,ComboAnimation=,AssociatedSoundCue=,bFullBody=false,bCombo=false,bLoop=false,bForce=false,fModifiedMovement=1.0,fAnimationLength=0.0,fBlendInTime=0.0,fBlendOutTime=0.00,bLastAnimation=false)
	...
	LightTorchInfo=(AnimationName=3p_1hsharp_throwdowntoup,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.BroadDagger_windup',bFullBody=False,bCombo=False,bLoop=False,bForce=false,fModifiedMovement=0.7,fAnimationLength=1.0,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false)
	ForceLightInfo=(AnimationName=3p_1hsharp_throwdowntoup,ComboAnimation=,AssociatedSoundCue=SoundCue'A_Combat_Locomotion.BroadDagger_windup',bFullBody=True,bCombo=False,bLoop=False,bForce=false,fModifiedMovement=0.0,fAnimationLength=1.0,fBlendInTime=0.00,fBlendOutTime=0.00,bLastAnimation=false)
